...
export const SlateCommand: FC<any> = ({ children, ...props }) => {
  ...
  const menuListBorderColor = useColorModeValue(
    "1px solid #ddd",
    "1px solid #444"
  );
  ...
  useEffect(() => {
    ...
    el.style.top = `${
      rect.top + window.pageYOffset - el.offsetHeight + elementHeight + 5
    }px`;
          ...
          if (e.key === CMD_KEY) {
            ...
            if (confirmEditor(e)) {
              ...
              if (
                ...
                editor.toggleBlock(
                  editor.commands[editor.selectedCommand].type
                );
              editor.deleteCurrentNodeText(commandOffset.current);
              closeTagSelectorMenu();
            }
          ...
          if (e.key === "Tab" || e.key === "ArrowDown") {
            ...
            if (
              ...
              const matchedCommands = matchSorter(
                SlateMenus,
                commandTextRef.current.substring(
                  1,
                  commandTextRef.current.length
                ),
                {
                  keys: ["type", "name"],
                }
                  ...
                  commands.map((x, idx) => {
                    ...
                    return (
                      <Box key={x.type + "-Box"}>
                        ...
                        {x.divider && (
                          <MenuDivider key={x.type + "commandMenuDivider"} />
                        )}
                      </Box>
                    );
                  })
